...
class LeftContiguousCSR:
    def __init__(
        self, indptr: np.ndarray, degrees: np.ndarray, data: np.ndarray
    ):
        self.indptr = indptr
    ...
    def save(self, filepath):
        np.savez(
            filepath, indptr=self.indptr, degrees=self.degrees, data=self.data
        )

    @staticmethod
    def load(filepath):
        npzfile = np.load(filepath)
        return LeftContiguousCSR(
            npzfile["indptr"], npzfile["degrees"], npzfile["data"]
        )

    @staticmethod
    def join(first, second):
        indptr = np.concatenate(
            [first.indptr, second.indptr + len(first.data)]
        )
...
class KGProcessedDataset:
    ...
    def get_entity_features(
        ...
        self.entity_feat = np.zeros(
            (self.num_entities, hidden_size), dtype=np.float64
        )
        ...
        for i in tqdm(range(0, self.num_entities, embedding_batch_size)):
            ...
            self.entity_feat[batch_slice] = (
                dp_model(**inputs)[0][:, 0, :].cpu().numpy()
            )

        self.relation_feat = np.array(
            [
                np.array(pipeline(self.get_first_n_words(e)))[
                    0, 0, :
                ].flatten()
...
class StATIKAdapter:
    ...
    def create_processed_dataset(
        ...
        entity_text = np.array(
            [
                str(node)
                for node in sorted(
                    entity_to_idx.keys(), key=lambda x: entity_to_idx[x]
                )
            ]
        )
        relation_text = np.array(
            [
                rel
                for rel in sorted(
                    relation_to_idx.keys(), key=lambda x: relation_to_idx[x]
                )
            ]
        )
    ...
    def _create_filtered_candidates(
        ...
        candidate_filter = np.ones(
            (queries.shape[0], num_entities), dtype=bool
        )
