...
def sample_and_get_largest_component(
    kg: KnowledgeGraph, n_edges: int | float, seed: int | None = None
) -> KnowledgeGraph:
    """Takes a random sample of edges from a KnowledgeGraph, creates a subgraph,
    and returns its largest weakly connected component as a new KnowledgeGraph.
    ...
    if isinstance(n_edges, float):
        if not 0 < n_edges <= 1:
            raise ValueError(
                "If n_edges is a float, it must be between 0 and 1"
            )
...
def remove_rare_relations(kg: KnowledgeGraph, n) -> KnowledgeGraph:
    """Remove edges from graph if their 'relation' attribute appears less than n times,
    ...
    rare_relations = {
        rel for rel, count in relation_counts.items() if count < n
    }

    # Remove edges with rare relations
    edges_to_remove = []
    for u, v, data in kg_copy.edges(data=True):
...
def filter_graph(
    ...
    if filter_criterion is None and attr_name is not None:
        filter_criterion = lambda d: d.get(attr_name) == attr_value

    if filter_criterion is None:
        raise ValueError(
            "Must provide either filter_criterion or attr_name/attr_value"
        )
