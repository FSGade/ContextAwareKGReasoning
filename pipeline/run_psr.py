...
Probability methods (--method flag):
  - path_noisy_or  : Original PSR noisy-OR over paths (may inflate at 3-hop
                     due to shared undirected variables across paths)
  - bdd_exact      : Exact probability via BDD per metapath group — handles
                     shared variables correctly.  Requires `pip install dd`.
  - monte_carlo    : Unbiased MC estimate per metapath group — handles shared
                     variables correctly.  No external deps.
  - all            : Run all three; BDD used as primary `probability` column.
                     Additional columns: prob_path_noisy_or, prob_bdd, prob_mc
...
Usage:
    ...
    python run_psr.py --tissue visceral --hops 3 --method bdd_exact --config config.yaml
    python run_psr.py --tissue white --hops 2 --method all --config config.yaml
"""
...
# Multi-hop methods with shared-variable awareness
...
# ---------------------------------------------------------------------------
# Edge attribute helpers (unchanged)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Path enumeration (unchanged — collects all edge attributes)
# ---------------------------------------------------------------------------
        ...
        for intermediate, e1 in outgoing[source]:
            ...
            for target, e2 in outgoing[intermediate]:
                ...
                paths.append({
                    ...
                    # Node sequence for BDD/MC (NEW)
                    'node_path': [source, intermediate, target],
                })
            ...
            for int2, e2 in outgoing[int1]:
                ...
                for target, e3 in outgoing[int2]:
                    ...
                    paths.append({
                        ...
                        # Node sequence for BDD/MC (NEW)
                        'node_path': [source, int1, int2, target],
                    })
...
# ---------------------------------------------------------------------------
# Probability computation per metapath group
# ---------------------------------------------------------------------------
def _compute_path_noisy_or(path_probs: List[float]) -> float:
    """Original PSR: 1 - prod(1 - p_path) over paths in a group."""
    if not path_probs:
        return 0.0
...
def compute_group_probabilities(
    ...
    method: str,
    sg: SupportGraph = None,
    mc_n_samples: int = 5000,
    mc_seed: int = 42,
    mc_alpha: float = 0.05,
) -> Dict[Tuple, Dict[str, Any]]:
    """
    Compute probability for each (source, target, metapath) group using
    the requested method.

    Args:
        grouped: {(source, target, metapath): [path_dicts]}
        method: 'path_noisy_or', 'bdd_exact', or 'monte_carlo'
        sg: SupportGraph (required for bdd_exact and monte_carlo)
        mc_n_samples: MC sample count
        mc_seed: MC RNG seed
        mc_alpha: MC confidence level

    Returns:
        {group_key: {'probability': float, ...extra_info...}}
    """
    results = {}
    ...
    if method == 'path_noisy_or':
        for key, plist in grouped.items():
            ...
            results[key] = {
                'probability': _compute_path_noisy_or(probs),
            }

    elif method == 'bdd_exact':
        if sg is None:
            raise ValueError("SupportGraph required for bdd_exact")
        ...
        for idx, (key, plist) in enumerate(grouped.items()):
            if (idx + 1) % 10000 == 0:
                print(f"    BDD progress: {idx + 1:,} / {n_groups:,} groups")
        ...
        if sg is None:
            raise ValueError("SupportGraph required for monte_carlo")

        # Collect node paths per group
        groups_node_paths = {
            key: [p['node_path'] for p in plist]
            for key, plist in grouped.items()
        }

        print(f"    Running MC with {mc_n_samples:,} samples "
              f"over {len(groups_node_paths):,} groups...")
        ...
        for key in grouped:
            ...
            results[key] = {
                ...
                'mc_hits': mc_r.get('hits'),
                'mc_n_samples': mc_r.get('n_samples'),
            }
...
def aggregate_paths(
    ...
    Aggregation rules:
    - Probability: computed by the selected method (BDD, MC, or path noisy-OR)
    ...
    When method='all', computes all three and stores them as separate columns,
    with BDD as the primary 'probability'.
    """
    if not paths:
        return pd.DataFrame()
    ...
    if method == 'all':
        ...
        print("  Computing path_noisy_or probabilities...")
        ...
        elif not _BDD_AVAILABLE:
            print("  WARNING: BDD not available, skipping bdd_exact")
        elif sg is None:
            print("  WARNING: SupportGraph not built, skipping bdd_exact")
    ...
    for (source, target, metapath), plist in grouped.items():
        ...
        # Evidence & coverage (method-independent)
        ...
        # Assign probability based on method
        if method == 'all':
            # BDD as primary (falls back to path_noisy_or if BDD unavailable)
            if key in probs_bdd:
    ...
    # Summary stats when running all methods
    if method == 'all' and 'prob_path_noisy_or' in df.columns and 'prob_bdd' in df.columns:
        _print_method_comparison_summary(df)

    return df


def _print_method_comparison_summary(df: pd.DataFrame):
    """Print a quick summary comparing probability methods."""
    pno = df['prob_path_noisy_or']
    bdd = df.get('prob_bdd')
    mc = df.get('prob_mc')

    print(f"\n  Method comparison summary:")
    print(f"    path_noisy_or: mean={pno.mean():.6f}, median={pno.median():.6f}")
    if bdd is not None and not bdd.isna().all():
        print(f"    bdd_exact:     mean={bdd.mean():.6f}, median={bdd.median():.6f}")
        ratio = (pno / bdd.clip(lower=1e-15))
        ratio_valid = ratio[bdd > 0]
        if len(ratio_valid) > 0:
            print(f"    inflation ratio (pno/bdd): "
                  f"mean={ratio_valid.mean():.3f}, "
                  f"median={ratio_valid.median():.3f}, "
                  f"max={ratio_valid.max():.3f}")
    if mc is not None and not mc.isna().all():
        print(f"    monte_carlo:   mean={mc.mean():.6f}, median={mc.median():.6f}")


# ---------------------------------------------------------------------------
# Subgraph & topic saving (unchanged)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# SupportGraph construction from aggregated KG
# ---------------------------------------------------------------------------
def build_support_graph_from_aggregated(
    kg: KnowledgeGraph,
    config: dict,
) -> SupportGraph:
    """
    Build a SupportGraph from the already-aggregated KnowledgeGraph.

    The aggregated graph has one edge per (u,v) pair, so each arc gets
    exactly one support variable.  Undirected edges (direction='0') produce
    a single shared variable that supports both arc directions — this is
    the key structural property that lets BDD/MC handle shared variables
    correctly.

    consider_undirected is set True to match the behaviour of
    _build_adjacency(), which expands undirected edges both ways.
    """
    psr_params = config.get('psr_params', {})
    consider_undirected = True  # match _build_adjacency behaviour

    print("Building SupportGraph from aggregated graph...")
    sg = build_support_graph(
        kg,
        consider_undirected=consider_undirected,
        base_edges_only=False,  # already aggregated — no inferred edges
...
def main():
    ...
    parser.add_argument('--method', type=str, default='path_noisy_or',
                        choices=['path_noisy_or', 'bdd_exact', 'monte_carlo', 'all'],
                        help='Probability computation method. '
                             'path_noisy_or = original PSR (default). '
                             'bdd_exact = exact via BDD (requires dd). '
                             'monte_carlo = unbiased MC estimate. '
                             'all = run all three, BDD as primary.')
    ...
    parser.add_argument('--mc-seed', type=int, default=42,
                        help='MC random seed (default: 42)')
    parser.add_argument('--mc-alpha', type=float, default=0.05,
                        help='MC confidence interval alpha (default: 0.05)')
    ...
    needs_bdd = args.method in ('bdd_exact', 'all')
    if args.method == 'bdd_exact' and not _BDD_AVAILABLE:
        print("ERROR: --method bdd_exact requires the 'dd' package.")
        print("Install with: pip install dd")
        print("Or use --method monte_carlo or --method path_noisy_or")
        sys.exit(1)
    ...
    # Build SupportGraph if needed
    needs_sg = args.method in ('bdd_exact', 'monte_carlo', 'all')
    sg = None
    if needs_sg:
        sg = build_support_graph_from_aggregated(kg, config)

    # Enumerate paths (collects all edge attributes)
    start = datetime.now()
    if args.hops == 2:
    ...
    # Aggregate with selected method
    df = aggregate_paths(
        paths, kg,
        method=args.method,
        sg=sg,
