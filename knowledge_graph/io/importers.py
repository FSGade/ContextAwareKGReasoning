...
class GraphImporter:
    ...
    def import_pickle(
        ...
        try:
            ...
            raise pickle.UnpicklingError(
                f"Error unpickling file at {file_path}: {str(exc)}"
            )
        except Exception as exc:
            raise Exception(
                f"An error occurred while loading the graph: {str(exc)}"
            )
    ...
    def import_gexf(
        ...
        node_types = {
            data.get("type", "Entity") for _, data in graph.nodes(data=True)
        }
        edge_types = {
