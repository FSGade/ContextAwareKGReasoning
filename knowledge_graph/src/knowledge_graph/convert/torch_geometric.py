...
def edges_iter(G, with_rev=False):
    ...
    Args:
        ...
        with_rev (bool, optional): _description_. Defaults to False.
...
def to_hetero_torch_geometric(
    ...
    for edge_group, group_edges in group_to_edges.items():
        ...
        hetero_data_dict[group_name] = {
            k: v
            for k, v in _get_edge_attributes(G_edges, edge_indexes=group_edges).items()
    ...
    if graph_attrs is not None:
        graph_items = {k: v for k, v in graph_items.items() if k in graph_attrs}
    ...
    for group, group_dict in hetero_data_dict.items():
        if isinstance(group_dict, dict):
            xs = []
            is_edge_group = group in ["__".join(k) for k in group_to_edges.keys()]
            if is_edge_group:
            ...
            for key, value in group_dict.items():
                if isinstance(value, (tuple, list)) and any(
                    isinstance(_value, torch.Tensor) for _value in value
                ):
                    value_0 = next(
                        (x for x in value if isinstance(x, torch.Tensor)), None
                    )
                    value_zero_imputed = [
                        (
                            v
                ...
                if (
                    ...
                    and (
                        key in group_attrs
                        or (group_attrs == "all" and key not in ("name", "type"))
                    )
                    and (group_attrs_exclude is None or key not in group_attrs_exclude)
                ):
                    _x = value
            ...
            if group_attrs is not None:
                if len(group_attrs) != len(xs) and group_attrs != "all":
                    ...
                    raise KeyError(f"Missing required attribute in group: {group}")
                if is_edge_group:
                    hetero_data_dict[group]["edge_attr"] = torch.cat(xs, dim=1)
                else:
                    for _x in xs:
                        print(_x.shape if isinstance(_x, torch.Tensor) else len(_x))
