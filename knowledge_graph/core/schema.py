...
class KnowledgeGraphSchema:
    ...
    def __init__(
        ...
        self._edge_type_usage_count: defaultdict[EdgeTypeUsage, int] = (
            defaultdict(int)
        )
    ...
    def __str__(self) -> str:
        ...
        for ntype in sorted(self._node_type_count):
            lines.append(
                f"    - {ntype} (count: {self._node_type_count[ntype]})"
            )
        ...
        for source_type in sorted(usage_by_source):
            ...
            for usage, count in sorted(
                ...
                lines.append(
                    f"      - {usage.edge_type} → {usage.target_type} "
                    f"(count: {count})"
                )
    ...
    def __repr__(self) -> str:
        ...
        return (
            ...
            f"node_types={repr(set(self._node_type_count.keys()))}, "
            f"edge_types={repr(self._allowed_edge_types)}, "
    ...
    def _group_usage_by_source(
        ...
        usage_by_source: dict[str, list[tuple[EdgeTypeUsage, int]]] = (
            defaultdict(list)
        )
    ...
    def add_node_type(self, node_type: str) -> None:
        ...
        if self._frozen:
            raise ValueError(
                f"Cannot add node type '{node_type}': schema is frozen"
            )
    ...
    def add_edge_type(self, edge_type: str) -> None:
        ...
        if self._frozen:
            raise ValueError(
                f"Cannot add edge type '{edge_type}': schema is frozen"
            )
    ...
    def remove_node_type(self, node_type: str) -> None:
        ...
        if self._frozen:
            raise ValueError(
                f"Cannot remove node type '{node_type}': schema is frozen"
            )
        if self._node_type_count[node_type] > 0:
            raise ValueError(
                f"Cannot remove node type '{node_type}': still in use"
            )
    ...
    def remove_edge_type(self, edge_type: str) -> None:
        ...
        if self._frozen:
            raise ValueError(
                f"Cannot remove edge type '{edge_type}': schema is frozen"
            )
        if any(
            ...
            raise ValueError(
                f"Cannot remove edge type '{edge_type}': still in use"
            )
    ...
    def visualize(
        ...
        # Add edge labels
        edge_labels = {
            (u, v): d.get("type", "") for u, v, d in kg.edges(data=True)
        }
